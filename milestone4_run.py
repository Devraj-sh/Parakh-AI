"""
milestone4_run.py — MILESTONE 4 runner.

Demonstrates confidence scoring formula calculation for extracted fields.
"""
import json
import sys
...
def run():
    print("=" * 70)
    print("  PARAKH AI — MILESTONE 4: Weighted Confidence Scoring Formula")
    print("=" * 70)
    ...
    for field_name in ["net_quantity", "mrp", "manufacturer", "batch_number", "date_information"]:
        ...
        scored_fields[field_name] = {
            "data": {k: v for k, v in field_data.items() if k not in ["anchor", "raw_text", "match_score"]},
            "confidence": conf,
        }

    print("\n  ── Field Confidence Breakdown ──────────────────────────────────")
    print(json.dumps(scored_fields, indent=2))
    print(f"\n{'='*70}")
    print("✅  MILESTONE 4 DONE — Weighted confidence formula populated.")
    print("    Proceed to Milestone 5 (evidence region assembly).")
    print("=" * 70)
